...
Telegram messages begin directly with their useful content instead of a sender header. AI replies end with **This is an automated AI-generated message based on the information provided.** Coach Patrick replies have no personal-attribution footer. After a substantive Coach reply, a short operational footer explains that the parent can continue typing, use `/status`, or use `/close`. `/status` uses parent-friendly wording such as **Escalated to Coach Patrick** instead of internal state names, while `/close` is accepted only after a complete Coach reply. `/help` explains the current flow; `/human` and other unknown commands cannot bypass the AI-first confirmation. The `/start` response tells parents to type their question and does not show a decision button. Resolution feedback is delayed until the third actual AI answer instead of appearing after every response. During a confirmed escalation or Coach Patrick takeover, the AI remains paused, including when the parent sends `/start` or a non-text message.
