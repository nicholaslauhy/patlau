...
An optional private Telegram supergroup forum provides a structured Coach inbox. Topics must be enabled, the parent-support bot must be an administrator with **Manage Topics** and **Delete Messages**, and **Remain anonymous** must stay off. Each escalated parent receives one private topic named for the parent, with one clean status icon: red needs attention, yellow is in progress, and green is complete. An authorised administrator can type a normal message in that topic; the webhook authorises the individual sender's positive Telegram ID, checks the mapped conversation and latest parent turn, makes retries idempotent, and lets the first administrator own that turn. The parent's Telegram receives only the administrator's message exactly as written. When a photo causes or follows an escalation, the bot reuses Telegram's private file reference to show one protected copy inside that parent's forum topic, with the reason and secure app/website links in its caption. The photo is never exposed through a public URL or copied into application logs. Telegram's protected-content setting discourages forwarding and saving but cannot prevent screenshots, so membership of the private forum must remain restricted to authorised support administrators.
