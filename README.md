...
Outbound bubbles in `/chats` show Telegram delivery honestly: one tick means Telegram accepted the message, while two ticks mean the parent sent a later reply. Telegram's Bot API does not expose passive read receipts, so the interface never labels inferred parent activity as `Read`. This is derived from the existing private message sequence and requires no additional database column.

Telegram's Bot API also does not provide a parent's online status, last-seen time, or incoming typing activity. The website therefore shows only relative **last parent message** activity calculated from the newest stored inbound message; this must not be interpreted as proof that the parent is currently online. Telegram message edits and deletions are not mirrored into the private support forum, so a mirrored topic message represents what PatLau received at delivery time.
