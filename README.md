...
An optional private Telegram supergroup forum provides a structured Coach inbox. Topics must be enabled, the parent-support bot must be an administrator with **Manage Topics**, **Delete Messages**, and **Pin Messages**, and **Remain anonymous** must stay off. Every stored conversational parent text message or Telegram photo is mirrored into that parent's private topic, whether the AI is still handling the conversation or it has been escalated. Parent copies start with the parent's single-line Telegram display name, while AI and system copies retain their existing **AI assistant:** and **System:** labels. When a topic is first provisioned, its first bot-authored message contains only the secure **Open in PatLau app** and **Open on website** links and is pinned; the parent's first turn is sent separately immediately afterwards. Each parent has one topic named for them, with one clean status icon: red needs attention, yellow is in progress, and green is complete. In a Coach-active topic, an authorised administrator can type a normal message; the webhook authorises the individual sender's positive Telegram ID, checks the mapped conversation and latest parent turn, makes retries idempotent, and lets the first administrator own that turn. The parent's Telegram receives only the administrator's message exactly as written. For a photo, the bot reuses Telegram's private file reference to show one protected copy inside the correct parent's forum topic. The photo is never exposed through a public URL or copied into application logs. Telegram's protected-content setting discourages forwarding and saving but cannot prevent screenshots, so membership of the private forum must remain restricted to authorised support administrators.
...
Outbound bubbles in `/chats` use the same compact delivery states as the app: `... Sending`, `✓ Sent`, `✓✓ Received`, and `! Failed`. **Sent** means Telegram accepted the outbound message. **Received** appears only after the parent sends a later message or directly replies, which confirms subsequent parent activity; Telegram's Bot API does not expose passive read receipts. This requires no additional database column.

The website message panel preserves its position during the ten-second conversation refresh. It follows new messages only while already near the bottom; otherwise a numbered **new messages below** control appears without interrupting what the superuser is reading. **Bring to top** and **Bring to bottom** buttons provide explicit navigation within the conversation without scrolling the surrounding page.

`TELEGRAM_PARENT_SUPPORT_FORUM_CHAT_ID` stores the forum's separate negative `-100...` supergroup ID. After the forum-enabled deployment is live, an existing authorised administrator can send `/forumid` inside the private forum to obtain it. If the forum is unconfigured, still provisioning, unavailable, or rejects an alert, PatLau keeps the existing private per-administrator alert and ForceReply workflow as a safe fallback. Parent commands, AI replies, system replies, and website replies are mirrored into an existing topic, status changes rename/close/reopen it, and permanent conversation deletion removes the Telegram topic before deleting Supabase history. The pinned header and full message mirroring reuse the existing `header_message_id` and per-message notification records, so this update does not require another Supabase migration.
